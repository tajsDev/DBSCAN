...
//kernel prototypes
__global__ void getNeighborFreqs(float *dataset, float eps, unsigned int N, int DIM, int min_pts, int *neighborFreqs, int *neighborsArr, int *neighborPos);
__global__ void getNeighborsArr(float *sortedD, float eps, unsigned int N, int DIM, int min_pts, int *neighborFreqs, int *neighborsArr, int *neighborPos);

__global__ void getNeighborFreqsSorted(float *sortedD, float eps, unsigned int N, int DIM, int min_pts, int *neighborFreqs, int *neighborsArr, int *neighborPos);
__global__ void getNeighborsArrSorted(float *sortedD, float eps, unsigned int N, int DIM, int min_pts, int *neighborFreqs, int *neighborsArr, int *neighborPos);
...
int main(int argc, char *argv[])
{
  ...
  if (MODE == 0 || MODE == 1)  //change this when we get sorting dataset done
  {
...
*/
__global__ void getNeighborFreqs(float *dataset, float eps, unsigned int N, int DIM, int min_pts, int *neighborFreqs, int *neighborsArr, int *neighborPos) //called with thread per element of dataset (N threads)
{
	...
	for (int pointIndex=0; pointIndex < N && numNeighbors < MAX_NEIGHBORS; pointIndex++)            //can optimize by having neighbors >= minpts terminate loop
	{
		float currSumOfDiff = 0;
		...
		{
			currSumOfDiff += (dataset[tid*DIM+dimIndex] - dataset[pointIndex*DIM+dimIndex]) * 
                                         (dataset[tid*DIM+dimIndex] - dataset[pointIndex*DIM+dimIndex]);
		}
...
__global__ void getNeighborFreqsSorted(float *sortedD, float eps, unsigned int N, int DIM, int min_pts, int *neighborFreqs, int *neighborsArr, int *neighborPos) //called with thread per element of dataset (N threads)
{
	//assign thread ID 0,1,2,3....N
	unsigned int tid = threadIdx.x + blockIdx.x * blockDim.x;
